<x-layout>
    ...
    <x-slot:heading>
        <h2 class="text-2xl uppercase font-bold">{{ $recipe->name }}</h2>
    </x-slot:heading>
    ...
    <section class="w-full items-center px-3">
        ...
        <article class="flex flex-col shadow my-4">
            ...
            <div class="bg-white flex flex-col justify-start p-6">
                ...
                <p class="text-3xl font-bold pb-4 first-letter:text-7xl capitalize">{{ $recipe->name }}</p>
                <p class="text-sm pb-2">
                    Від <a href="{{ route('user.recipes', $recipe->user) }}"
                        class="font-semibold hover:text-gray-800">{{ $recipe->user->name }}</a>
                ...
                @auth
                    @if (auth()->id() !== $recipe->user_id)
                        <div x-data="{
                            ...
                            <svg @click="toggleLike" height="40px" width="40px" version="1.1" id="Layer_1"
                                xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                                viewBox="0 0 511.999 511.999" xml:space="preserve" fill="#000000">
                                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                <g id="SVGRepo_iconCarrier">
                                    <path class="hover:cursor-pointer"
                                        :class="liked ? 'fill-current text-red-500 hover:text-red-700' :
                                            'fill-current text-white hover:text-red-300'"
                    ...
                    @else
                        <div class="mt-4 flex items-center">
                            <svg height="40px" width="40px" version="1.1" id="Layer_1"
                                xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                                viewBox="0 0 511.999 511.999" xml:space="preserve" fill="#B91C1C">
                                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                <g id="SVGRepo_iconCarrier">
                                    <path
                    ...
                    <form action="{{ route('comments.store', $recipe) }}" method="POST">
                        ...
                        <div class="mb-4">
                            <textarea id="body" name="body" rows="4"
                                class="mt-1 p-4 block w-full shadow-sm sm:text-sm border border-gray-300 rounded-md" required></textarea>
                            @error('body')
        ...
        <div class="w-full flex pt-6">
            @if ($previousRecipe)
                <a href="{{ route('recipes.show', $previousRecipe) }}"
                    ...
                    <p class="text-lg text-blue-800 font-bold flex items-center"><i
                        ...
                        Previous</p>
                    <p class="pt-2">{{ $previousRecipe->name }}</p>
                </a>
            @else
                <div class="w-1/2"></div>
            @endif
            ...
            @if ($nextRecipe)
                <a href="{{ route('recipes.show', $nextRecipe) }}"
                    ...
                    <p class="pt-2">{{ $nextRecipe->name }}</p>
                </a>
            @else
                <div class="w-1/2"></div>
            @endif
