    ...
    <form action="{{ route('recipes.store') }}" method="POST" enctype="multipart/form-data" class="max-w-4xl mx-auto px-8">
        ...
        <div class="mb-5">
            ...
            <textarea id="ingredients" name="ingredients"
                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                placeholder="молоко, яйця, борошно, сіль, вершкове масло" required>{{ old('ingredients') }}</textarea>
            @error('ingredients')
                <span class="mt-4 text-red-500 text-sm">{{ $message }}</span>
            @enderror
        </div>
        ...
        <button type="submit"
            class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center">Створити</button>
    </form>
